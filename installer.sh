...
echo "Starting SLwebcams installation..."

# Determine plugin path based on architecture
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/SLwebcams
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/SLwebcams
fi
...
cleanup() {
    ...
    [ -d "/tmp/SLwebcams-main" ] && rm -rf "/tmp/SLwebcams-main"
}
...
echo "Downloading SLwebcams..."
wget --no-check-certificate 'https://github.com/OwnerPlugins/SLwebcams/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "Failed to download SLwebcams package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to extract SLwebcams package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/SLwebcams-main/usr/lib/enigma2/python/Plugins/Extensions/SLwebcams" ]; then
    cp -r "$TMPPATH/SLwebcams-main/usr/lib/enigma2/python/Plugins/Extensions/SLwebcams"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/SLwebcams-main/usr/lib64/enigma2/python/Plugins/Extensions/SLwebcams" ]; then
    cp -r "$TMPPATH/SLwebcams-main/usr/lib64/enigma2/python/Plugins/Extensions/SLwebcams"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/SLwebcams-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/SLwebcams-main/usr"/* /usr/ 2>/dev/null
...
#########################################################
#        SLwebcams $version INSTALLED SUCCESSFULLY      #
