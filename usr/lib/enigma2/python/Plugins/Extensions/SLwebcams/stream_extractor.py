...
class StreamExtractor:
    ...
    def extract_stream_url(self, webcam_url):
        ...
        """
        enhanced_log("Extracting stream URL from: {}".format(
            webcam_url), processo="extract_stream_url")
        html_content = self._fetch_url(webcam_url)

        if not html_content:
            enhanced_log("No HTML content found for: {}".format(
                webcam_url), "WARNING", processo="extract_stream_url")
            return None
        ...
        if m3u8_match:
            ...
            final_url = "https://hd-auth.skylinewebcams.com/live.m3u8?a={}".format(
                token)
            enhanced_log(
                "Found m3u8 URL with token: {}".format(final_url),
        ...
        if m3u8_full_match:
            ...
            enhanced_log(
                "Found full m3u8 URL: {}".format(final_url),
                processo="extract_stream_url")
            return final_url
        ...
        if iframe_match:
            ...
            enhanced_log(
                "Found iframe: {}".format(iframe_url),
                processo="extract_stream_url")
            ...
            enhanced_log("Extracting stream from iframe: {}".format(
                iframe_url), processo="extract_stream_url")
    ...
    def _extract_stream_from_iframe(self, iframe_url):
        ...
        Returns:
            str: The extracted stream URL, or None if not found
        """
        enhanced_log("Analyzing iframe: {}".format(iframe_url),
                     processo="_extract_stream_from_iframe")
        html_content = self._fetch_url(iframe_url)

        if not html_content:
            enhanced_log("No HTML content found in iframe: {}".format(
                iframe_url), "WARNING", processo="_extract_stream_from_iframe")
            return None
        ...
        if m3u8_token_match:
            ...
            final_url = "https://hd-auth.skylinewebcams.com/live.m3u8?a={}".format(
                token)
            enhanced_log("Found m3u8 URL with token in iframe: {}".format(
                final_url), processo="_extract_stream_from_iframe")
            return final_url
        ...
        if m3u8_match:
            enhanced_log(
                "Found m3u8 URL in iframe: {}".format(
                    m3u8_match.group(1)),
                processo="_extract_stream_from_iframe")
            return m3u8_match.group(1)
        ...
        if mp4_match:
            enhanced_log(
                "Found MP4 URL in iframe: {}".format(
                    mp4_match.group(1)),
                processo="_extract_stream_from_iframe")
        ...
        if stream_match:
            enhanced_log(
                "Found generic stream URL in iframe: {}".format(
    ...
    def _fetch_url(self, url):
        ...
        """
        enhanced_log(
            "Fetching content from: {}".format(url),
            processo="_fetch_url")
        try:
            ...
            with urllib.request.urlopen(request, context=self.context) as response:
                enhanced_log(
                    "Content fetched successfully from: {}".format(url),
                    processo="_fetch_url")
