...
class StreamExtractor:
    ...
    def extract_stream_url(self, webcam_url):
        ...
        """
        enhanced_log(f"Extracting stream URL from: {webcam_url}", processo="extract_stream_url")
        html_content = self._fetch_url(webcam_url)

        if not html_content:
            enhanced_log(f"No HTML content found for: {webcam_url}", "WARNING", processo="extract_stream_url")
            return None
        ...
        if m3u8_match:
            ...
            enhanced_log(f"Found m3u8 URL with token: {final_url}", processo="extract_stream_url")
            return final_url
        ...
        if m3u8_full_match:
            ...
            enhanced_log(f"Found full m3u8 URL: {final_url}", processo="extract_stream_url")
            return final_url
        ...
        if m3u8_match:
            enhanced_log(f"Found legacy m3u8 URL: {m3u8_match.group(1)}", processo="extract_stream_url")
        ...
        if rtmp_match:
            enhanced_log(f"Found RTMP URL: {rtmp_match.group(1)}", processo="extract_stream_url")
        ...
        if iframe_match:
            ...
            enhanced_log(f"Found iframe: {iframe_url}", processo="extract_stream_url")
            ...
            enhanced_log(f"Extracting stream from iframe: {iframe_url}", processo="extract_stream_url")
        ...
        if json_match:
            try:
                ...
                if 'stream' in webcam_data and webcam_data['stream']:
                    enhanced_log(f"Found stream in JSON: {webcam_data['stream']}", processo="extract_stream_url")
    ...
    def _extract_stream_from_iframe(self, iframe_url):
        ...
        Returns:
            str: The extracted stream URL, or None if not found
        """
        enhanced_log(f"Analyzing iframe: {iframe_url}", processo="_extract_stream_from_iframe")
        html_content = self._fetch_url(iframe_url)

        if not html_content:
            enhanced_log(f"No HTML content found in iframe: {iframe_url}", "WARNING", processo="_extract_stream_from_iframe")
            return None
        ...
        if m3u8_token_match:
            ...
            final_url = f"https://hd-auth.skylinewebcams.com/live.m3u8?a={token}"
            enhanced_log(f"Found m3u8 URL with token in iframe: {final_url}", processo="_extract_stream_from_iframe")
            return final_url
        ...
        if m3u8_match:
            enhanced_log(f"Found m3u8 URL in iframe: {m3u8_match.group(1)}", processo="_extract_stream_from_iframe")
            return m3u8_match.group(1)
        ...
        if mp4_match:
            enhanced_log(f"Found MP4 URL in iframe: {mp4_match.group(1)}", processo="_extract_stream_from_iframe")
        ...
        if stream_match:
            enhanced_log(f"Found generic stream URL in iframe: {stream_match.group(1)}", processo="_extract_stream_from_iframe")
    ...
    def _fetch_url(self, url):
        ...
        """
        enhanced_log(f"Fetching content from: {url}", processo="_fetch_url")
        try:
            ...
            with urllib.request.urlopen(request, context=self.context) as response:
                enhanced_log(f"Content fetched successfully from: {url}", processo="_fetch_url")
